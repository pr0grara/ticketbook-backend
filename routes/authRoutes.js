...
router.post('/login', async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, userId: user._id, firstname: user.firstname, watchedTutorial: user.watchedTutorial || false }).end();
    } catch {
...
router.post("/logout", (req, res) => {
    const isDevLogout = req.query.dev === "true";

    res.clearCookie("authToken", { 
        httpOnly: true, 
        secure: !isDevLogout,
        sameSite: !isDevLogout ? "None" : "Lax",
        path: "/"
        // domain: process.env.PROD_ENV === "true" ? "api.arabuilds.com" : "localhost",
    });
